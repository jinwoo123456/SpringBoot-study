...
public interface IMemberService {
	...
	//	public List<MemberDTO> select(MemberDTO memberDTO);
		...
		//회원등록 : request 내장객체를 통해 받은 파라미터를 개별적으로 전달.
		public int insert(String id,String pass, String name);
		//정보 조회
		/*회원정보 인출하기 : 파라미터를 받은 후 @Param 어노테이션으로 Mapper에서
		 * 사용할 파라미터명을 지정한다. 즉 id로 받은 후 _id로 변경해서 전달한다.*/
		public MemberDTO selectOne(@Param("_id") String id);
		//수정 처리 : 파라미터가 저장된 Map을 인수로 전달한다.
